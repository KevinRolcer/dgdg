    ...
    public function export(int $moduleId, string $format, ?array $exportConfig = null): array
    {
        ...
        foreach ($columnsCfg as $col) {
            ...
            $columnMap[$key] = [
                ...
                'image_width' => isset($col['image_width']) && is_numeric($col['image_width']) ? max(40, min(400, (int) $col['image_width'])) : null,
                'image_height' => isset($col['image_height']) && is_numeric($col['image_height']) ? max(30, min(300, (int) $col['image_height'])) : null,
                'image_fit_mode' => ((string) ($col['image_fit_mode'] ?? 'cover')) === 'contain' ? 'contain' : 'cover',
